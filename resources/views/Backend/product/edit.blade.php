...
@section('body')
    ...
    <div class="modal-content">
        <div class="modal-header ">
            <h2 class="modal-title text-center">Edit Product</h2>
            <button type="button" class="btn-close btn-danger" data-bs-dismiss="modal"  aria-label="Close"></button>
        </div>
        <div class="modal-body text-dark">
            <form action="{{ route('products.update',$product->id) }}" method="POST" class="text-dark" enctype="multipart/form-data" >
                @csrf
                <div class="row">
                    {{--                About Text--}}
                    <div class="col-md-10 mx-auto">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-8">
                                <textarea name="product_short_description" id="summernote" cols="100" rows="2">{!! $product->product_short_description !!} </textarea>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="about" class=" form-label fw-bolder">Product Long Description</label>
                            <div class="col-md-12">
                                <textarea name="product_long_description" id="summernote1" cols="100" rows="2">{!! $product->product_long_description !!}</textarea>
                            </div>
                        </div>
                        {{--                Image--}}
                        <div class="row mb-3">
                            ...
                            <img src="{{ asset( $product->image) }}" alt="" style="height: 80px; width: 120px;">
                        </div>
                        <div class="row mb-3">
                            <label for="" class="col-md-4 form-label fw-bolder">Product Other Image</label>
                            <div class="col-md-8">
                                <input type="file" class=" mb-2 btn btn-success form-control" name="other_image[]" multiple>
                                @foreach($product->otherImages as $otherImage)
                                    <img src="{{ asset( $otherImage->image) }}" alt="" style="height: 70px; width: 100px;">
                                @endforeach
                            </div>
